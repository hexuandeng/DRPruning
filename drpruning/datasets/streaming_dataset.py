...
from drpruning.datasets.partition import get_partitions_orig
...
def generate_work(dataset: StreamingDataset, 
    ...
    for stream_id, stream in enumerate(dataset.streams):
        ...
        dataset._set_shuffle_block_size()
        ...
        # assert np.intersect1d(partition_per_stream[-1].flatten(), np.array(used_sample_ids[stream_id])).size == 0
...
class DynamicStreamingDataset(StreamingDataset):
    ...
    def _get_work(self, world: World, epoch: int, used_sample_ids: List[List[int]]) -> NDArray[np.int64]:
        ...
        if world.is_local_leader:
            sample_ids_per_stream = generate_work(self, world, epoch)#, used_sample_ids)
        ...
        for stream_id in range(self.num_streams):
            count_used = Counter(used_sample_ids[stream_id])
            sample_ids_per_stream[stream_id] = [i for i in sample_ids_per_stream[stream_id] if i >= 0]
            self.used_num_samples_per_stream[stream_id] = sum(count_used[i] for i in sample_ids_per_stream[stream_id])
            if self.used_num_samples_per_stream[stream_id]:
                print(f"Node {world.node} Rank {world.rank_of_node} Worker {world.worker_of_rank} Stream {stream_id} Resume from dataset No.{self.used_num_samples_per_stream[stream_id]}.")
                print(used_sample_ids[stream_id][0], used_sample_ids[stream_id][-1])
                print(sample_ids_per_stream[stream_id][0], sample_ids_per_stream[stream_id][-1])
