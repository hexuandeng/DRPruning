...
from drpruning.callbacks.DRO_loading_callback import \
    DRPruningCallback
from drpruning.callbacks.pruning_callback import PruningCallback
from drpruning.datasets.load_text_dataloader import build_text_dataloader
from drpruning.models.model_registry import COMPOSER_MODEL_REGISTRY
...
def main(cfg):
    ...
    cfg.dist_timeout = cfg.get('dist_timeout', 5400.0)
    ...
    if data_loading_config.dynamic:
        if data_loading_config.update_type in ["sheared", "bandit", "constant"]:
            dl_callback = DynamicLoadingCallback(reference_loss=data_loading_config.reference_loss,
        ...
        elif "dro" in data_loading_config.update_type:
            dl_callback = DRPruningCallback(reference_loss=data_loading_config.reference_loss,
                                                proportion=data_loading_config.proportion,
                                                set_names=data_loading_config.set_names,
                                                rho=data_loading_config.rho,
                                                ema=data_loading_config.ema,
                                                min_prob=getattr(data_loading_config, 'min_prob', None),
                                                clamp_q_to_min=getattr(data_loading_config, 'clamp_q_to_min', None),
                                                dynamic_proportion="DyAll" in data_loading_config.update_type,
                                                dynamic_baseline="Dy" in data_loading_config.update_type,
                                                use_eval="Eval" in data_loading_config.update_type,
                                                for_prune=cfg.model.l0_module is not None,
    ...
    if data_loading_config.dynamic:
        ...
        from drpruning.datasets.state import _dataset_state_dict
        trainer.state._dataset_state_dict = MethodType(_dataset_state_dict, trainer.state)
        
    print('Logging config...')
    log_config(cfg)

    if cfg.get('eval_first', False) and "dro" not in data_loading_config.update_type:
        trainer.eval()

    print('Starting training...')
    trainer.state._dataloader_len = None
    trainer.fit()
