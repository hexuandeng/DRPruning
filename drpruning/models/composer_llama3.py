...
class ComposerMosaicLlama3(ComposerModel):
...
class Llama3Model(nn.Module):
    def __init__(self, cfg: DictConfig):
        ...
        self.transformer.update({
            'blocks':
                nn.ModuleList([
                    Llama3Block(cfg, device=cfg.init_device)
    ...
    def fsdp_wrap_fn(self, module):
        return isinstance(module, Llama3Block)
...
class Llama3Block(nn.Module):
    def __init__(self, cfg: DictConfig, device: Optional[str] = None):
        ...
        self.attn = Llama3Attention(cfg, device) 
...
class Llama3Attention(nn.Module):
    ...
    def __init__(self, cfg: DictConfig, device: Optional[str] = None):
        ...
        self.wq = nn.Linear(self.d_model, self.n_heads * self.head_dim, device=device, bias=False)
