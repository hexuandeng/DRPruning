...
def download_ShareGPT():
    ...
    tokenizer = AutoTokenizer.from_pretrained("../../LLMs/Llama-2-7b-hf")

    dataset = load_dataset('json', data_files='ShareGPT_V3_unfiltered_cleaned_split_no_imsorry.json')["train"]
    def reshape(data):
        data = data["conversations"]
        if len(data) >= 2 and data[0]["from"] == "human" and data[1]["from"] == "gpt":
            return {"instruction": "", "input": data[0]["value"], "output": data[1]["value"]}
        else:
            return {"instruction": "", "input": "", "output": ""}
    dataset = dataset.map(reshape, remove_columns=dataset.column_names)
    inputs = f"You are a helpful assistant. Write a response that appropriately completes the request.\n\n### Input:\n\n\n### Response:"
    max_len = 512 - len(tokenizer(inputs)['input_ids'])

    dataset = dataset.filter(lambda x: len(x["input"].strip().split()) > 4 and len(x["output"].strip().split()) > 4 and "sorry" not in x["output"])
    dataset = dataset.filter(lambda x: len(tokenizer(x["input"])['input_ids']) + len(tokenizer(x["output"])['input_ids']) < max_len)
    dataset = dataset.train_test_split(test_size=11010, seed=42)['test']
    dataset = dataset.train_test_split(test_size=1000, seed=42)
    dataset["train"].to_json("ShareGPT_train.json")
    dataset["test"].to_json("ShareGPT_test.json")
